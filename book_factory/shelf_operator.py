...
class BOOKGEN_OT_SelectShelf(bpy.types.Operator):
    ...
    bl_label = "Add Book Factory Shelf"
    ...
    def handle_confirm(self, context, mouse_x, mouse_y):
        ...
        if self.start is None:
            ...
            context.workspace.status_text_set(
                tr(context, "Move the mouse and click a surface to place the shelf endpoint")
            )
        ...
        source_settings = get_settings_for_new_grouping(context)
        settings = copy_bookgen_settings(context.scene, source_settings, f"{shelf_name}_Settings")
    ...
    def invoke(self, context, _event):
        ...
        context.workspace.status_text_set(
            tr(context, "Click a surface to place the shelf start; Esc to cancel")
        )
        return {"RUNNING_MODAL"}
