...
class Shelf:
    ...
    def add_book(self, book, offset, first=False):
        ...
        book.location += Vector((book.planar_offset.x, book.planar_offset.y, 0))
    ...
    def apply_parameters(self):
        ...
        planar_offset = 0.0
        if random.random() < p["shelf_offset_chance"]:
            maximum_offset = p["scale"] * p["shelf_depth_offset"]
            bias = p["shelf_offset_bias"]
            minimum = -maximum_offset * (1.0 - max(bias, 0.0))
