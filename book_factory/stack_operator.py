...
class BOOKGEN_OT_SelectStack(bpy.types.Operator):
    ...
    bl_label = "Add Book Factory Stack"
    ...
    def handle_confirm(self, context, mouse_x, mouse_y):
        ...
        settings = copy_bookgen_settings(context.scene, source_settings, f"{stack_name}_Settings")
    ...
    def invoke(self, context, _event):
        ...
        context.window.cursor_modal_set("CROSSHAIR")
        context.workspace.status_text_set(
            tr(context, "Click a surface to place the stack; Esc to cancel")
        )
        return {"RUNNING_MODAL"}
